...
import net.floodlightcontroller.unipi.amd.IAsynchronousMessageDelivery;
...
public class Forwarding extends ForwardingBase implements IFloodlightModule, IOFSwitchListener, ILinkDiscoveryListener,
    ...
    public Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision, FloodlightContext cntx) {
        ...
        // if receive an IP packet with a subscribed virtual ip as destination, it must be managed by the AsynchronousMessageDelivery module
        if (eth.getPayload() instanceof IPv4) {
        	IPv4 pkt = (IPv4)eth.getPayload();
        	log.info("S" + sw.getId().getLong() + " (FORWARDER) IPv4 packet from " + pkt.getSourceAddress().toString() + " to " + pkt.getDestinationAddress().toString());
    ...
    public void init(FloodlightModuleContext context) throws FloodlightModuleException {
        ...
        this.context = context;														// to retrieve the AMD interface in the processPacketInMessage
